...
import org.moosetechnology.verveineCore.gen.famix.UnknownContainerEntity;
...
//				parent = (ContainerEntity) resolveOrCreate(qualName.nameQualifiers(), /*mayBeNull*/false, UnknownContainerEntity.class);
		...
		if (isMethodBinding(bnd)) {
			...
			if (parent == null) {
				...
				if (QualifiedName.isFullyQualified(name)) {
					parent = (ContainerEntity) resolveOrCreate( QualifiedName.parentNameFromEntityFullname(name.toString()), /*mayBeNull*/false, org.moosetechnology.verveineCore.gen.famix.Class.class );
				}
				else {
	...
	public NamedEntity findInLocals(String name, ContainerEntity context) {
		for (Type child : context.getTypes()) {
